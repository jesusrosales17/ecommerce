...
export default function ReportPreviewPage() {
  ...
  React.useEffect(() => {
    async function fetchReportData() {
      ...
      try {
        ...
        const response = await fetch(`/api/admin/reports/${reportId}?dateRange=${dateRange}`);
  ...
  const getIcon = (category: string) => {
    switch (category) {
      case 'ventas': return <BarChart3 className="h-6 w-6" />;
      case 'clientes': return <Users className="h-6 w-6" />;
      case 'productos': return <Package className="h-6 w-6" />;
      case 'financiero': return <DollarSign className="h-6 w-6" />;
      case 'crecimiento': return <TrendingUp className="h-6 w-6" />;
  ...
  const handleDownload = async (format: 'pdf' | 'excel' | 'csv') => {
    try {
      ...
      a.href = url;
      a.download = `reporte-${reportId}-${new Date().toISOString().split('T')[0]}.${format}`;
      ...
      <div className="container mx-auto p-6">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">Cargando vista previa del reporte...</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="container mx-auto p-6">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-semibold mb-2">Error al cargar el reporte</h2>
      ...
      <div className="container mx-auto p-6 print:p-4">
        ...
        <div className="mb-8 print:mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold print:text-2xl">{reportInfo.title}</h1>
          </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 print:gap-4">
                ...
                {reportData.summary.conversionRate && (
                  <div className="text-center">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Conversión</p>
                  </div>
                )}
              </div>
            </CardContent>
              ...
              <CardContent>
                <div className="space-y-4 print:space-y-2">
                  {reportData.topProducts.slice(0, 10).map((product: any, index: number) => (
                    <div key={product.id} className="flex items-center justify-between p-4 rounded-lg border print:p-2 print:border-gray-300">
                      <div className="flex items-center gap-3">
                        <Badge variant="secondary" className="w-8 h-8 rounded-full flex items-center justify-center print:w-6 print:h-6">
                          {index + 1}
                        </Badge>
                        <div>
              ...
              <CardContent>
                <div className="space-y-4 print:space-y-2">
                  {reportData.topCustomers.slice(0, 10).map((customer: any, index: number) => (
                    <div key={customer.id} className="flex items-center justify-between p-4 rounded-lg border print:p-2 print:border-gray-300">
                      <div className="flex items-center gap-3">
                        <Badge variant="secondary" className="w-8 h-8 rounded-full flex items-center justify-center print:w-6 print:h-6">
                          {index + 1}
                        </Badge>
                        <div>
                          <p className="font-medium print:text-sm">{customer.name}</p>
                          <p className="text-sm text-muted-foreground print:text-xs">{customer.email}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-semibold print:text-sm">{formatCurrency(customer.totalSpent)}</p>
                        <p className="text-sm text-muted-foreground print:text-xs">
        ...
        <div className="mt-12 print:mt-8 pt-6 border-t text-center text-sm text-muted-foreground">
          <p>
            ...
            • Datos del período: {formatDateRangeText(dateRange || '')}
          </p>
        </div>
      </div>
    </div>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 print:gap-2">
                    {reportData.salesTrends.byCategory.map((category: any, index: number) => (
                      <div key={index} className="p-4 rounded-lg border print:p-2 print:border-gray-300">
                        <div className="flex justify-between items-center mb-2">
                          <p className="font-medium print:text-sm">{category.name}</p>
                          <Badge variant="outline" className="print:text-xs">
                            {formatCurrency(category.revenue)}
                          </Badge>
                        </div>
                        <div className="grid grid-cols-2 gap-2 text-sm print:text-xs">
                          <span className="text-muted-foreground">Cantidad vendida: {formatNumber(category.sales)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <CardContent>
                  <div className="space-y-3 print:space-y-2">
                    {reportData.customersByRegion.map((region: any, index: number) => (
                      <div key={index} className="flex items-center justify-between p-3 rounded-lg border print:p-2 print:border-gray-300">
                        <span className="font-medium print:text-sm">{region.region}</span>
                        <Badge variant="secondary">{region.customerCount} clientes</Badge>
                      </div>
                    ))}
                  </div>
                </CardContent>
                    ...
                    {reportData.categoryAnalysis.map((category: any, index: number) => (
                      <div key={index} className="p-4 rounded-lg border print:p-2 print:border-gray-300">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-sm print:text-xs">
                          <span className="text-muted-foreground">Productos: {category.productCount}</span>
                          <span className="text-muted-foreground">Cantidad: {formatNumber(category.quantity)}</span>
                          <span className="text-muted-foreground">Ingresos: {formatCurrency(category.revenue)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <CardContent>
                  <div className="space-y-3 print:space-y-2">
                    {reportData.profitMargins.map((margin: any, index: number) => (
                      <div key={index} className="flex items-center justify-between p-3 rounded-lg border print:p-2 print:border-gray-300">
                        <div>
                          <p className="font-medium print:text-sm">{margin.category}</p>
                          <p className="text-sm text-muted-foreground print:text-xs">
                            Beneficio: {formatCurrency(margin.profit)}
                          </p>
                        </div>
                        <Badge variant={margin.margin > 20 ? "default" : margin.margin > 10 ? "secondary" : "destructive"}>
                ...
                <CardContent>
                  <div className="space-y-3 print:space-y-2">
                    {reportData.statusBreakdown.map((status: any, index: number) => (
                      <div key={index} className="flex items-center justify-between p-3 rounded-lg border print:p-2 print:border-gray-300">
                        <div>
                          <p className="font-medium print:text-sm capitalize">{status.status.toLowerCase()}</p>
                          <p className="text-sm text-muted-foreground print:text-xs">
              ...
              <Card className="print:shadow-none print:border-gray-300 print:break-inside-avoid">
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 print:gap-2">
                    ...
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-600 print:text-lg">
                        {reportData.summary.orderGrowth > 0 ? '+' : ''}{reportData.summary.orderGrowth.toFixed(1)}%
                      </div>
                      <p className="text-sm text-muted-foreground mt-1">Órdenes</p>
                    </div>
                    <div className="text-center">
                    ...
                    <div className="text-center">
                      <div className="text-2xl font-bold text-orange-600 print:text-lg">
                        {reportData.summary.monthOverMonthGrowth > 0 ? '+' : ''}{reportData.summary.monthOverMonthGrowth.toFixed(1)}%
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 print:gap-2">
                    <div className="text-center">
                      <div className="text-xl font-bold text-green-600 print:text-lg">
                        {formatCurrency(reportData.projections.nextMonthRevenue)}
                      </div>
...
function getReportInfo(reportId: string) {
  const reportTypes: { [key: string]: any } = {
    'sales-summary': {
      ...
      description: 'Análisis completo del rendimiento de ventas, tendencias y métricas clave.',
      category: 'ventas'
    },
    'customer-analysis': {
      id: 'customer-analysis',
      title: 'Análisis de Clientes',
      description: 'Segmentación de clientes, comportamiento de compra y análisis de retención.',
      category: 'clientes'
    },
    'product-performance': {
      id: 'product-performance',
      title: 'Rendimiento de Productos',
      description: 'Análisis detallado del rendimiento por producto, categoría y tendencias.',
      category: 'productos'
    },
    'financial-report': {
      id: 'financial-report',
      title: 'Reporte Financiero',
      description: 'Estado financiero, márgenes, costos y análisis de rentabilidad.',
      category: 'financiero'
    },
    'orders-analysis': {
      id: 'orders-analysis',
      title: 'Análisis de Órdenes',
      description: 'Análisis del procesamiento de órdenes, tiempos y eficiencia operativa.',
